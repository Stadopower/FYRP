...
def visualise_csp(subject_id, raw, csp, y_train, y_pred, include_rest=True, kind='calibration'):
    ...
    plt.savefig(f"D:/RUG/Term 2/FYRP/data/ivo_data/figures/{subject_id}_confusion_matrix_{kind}.pdf")
    plt.close()

    csp.plot_filters(info=raw.pick('eeg').info, show_names=True, sphere='eeglab', colorbar=False)
    plt.savefig(f"D:/RUG/Term 2/FYRP/data/ivo_data/figures/{subject_id}_csp_filters_{kind}.pdf")
    plt.close()
...
def get_driving_epochs_for_csp(subject_id, include_rest=True, ica_kind='driving'):
    ...
    ica = read_ica(f'D:/RUG/Term 2/FYRP/data/ica/P{subject_id}-{ica_kind}-ica.fif')
